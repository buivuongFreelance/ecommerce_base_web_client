<script>
	...
	import { currency, selectedItemsCheckout, settings } from '../../stores';
...
<div>
	<h4 class="mb5">{$_('checkout.confirmCart')}</h4>
	...
	{#if listConfirmCheckout}
		...
		{/if}
	{/if}
	<div class="flex justify-end pa4 bb bl br b--light-gray bg-light-gray">
		{#if isLoadingShippo}
	...
	<div class="mt5">
		<h4 class="mb5">{$_('checkout.saleSummary')}</h4>
		<div class="bg-light-gray br3 pa4">
			<div class="flex justify-between mb3 light-silver">
			...
			<div class="flex justify-between mb3 light-silver">
				<span class="f6">{$_('cart.totalCommission')}</span>
				{#if isLoadingShippo}
					<LoadingDefault size={30} color="#e4e6ef" />
				{:else if sale.selectedRate}
					<span class="fw6">
		...
		<div
			class="flex justify-between items-center bt bb b--light-gray b--dashed pv4 ph4 mv4"
		>
			<div class="b">{$_('cart.total')}</div>
			<div class="f4 b">
				{#if isLoadingShippo}
					<LoadingDefault size={30} color="#e4e6ef" />
				{:else}
