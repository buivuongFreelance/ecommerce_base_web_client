	...
	const sellerPayShipping = () => {
		...
		return new Promise((resolve, reject) => {
			...
			apiSellerPayShipping(domainTransaction, {
				...
				shipping: detailShip,
				billing: isSameShipping ? detailBill : valuePayNew,
...
<div class="w-100 mt4">
	<h4 class="mb5">{$_('checkout.payment')}</h4>

	<form autocomplete="off" on:submit|preventDefault={handleSubmit} novalidate>
		<div class="br3 ba b--light-gray">
			<div
				class="h-50-px flex items-center pl4 bg-light-gray bb b--light-gray b f5"
			>
				{$_('checkout.cardInfo')}
			</div>
			<div class="ph4 pv4">
