	...
	{:else}
		<div bind:this={rootEle} class="dropzone">
			<div class="dz-message" data-dz-message>
				...
				<div class="f3 near-black fw6">{$_('image.dragDrop')}</div>
				<div class="mt3 steel-two f4 ttu">{$_('message.or')}</div>
				<div class="mt3 flex justify-center">
					<div
						class="h-60-px choose-files flex items-center justify-center pointer
							near-black f4 fw6 grow"
					>
						{$_('image.chooseFiles')}
					</div>
				</div>
				<div class="mt3 f4 fw6 steel-two">
					{$_('image.maximumMb', { values: { number: 10 } })}
				</div>
			</div>
		</div>
		<div class="mt5 flex justify-center items-center">
			{#if isLoadingDelete}
				<div class="box br2 flex items-center justify-center mr5 relative">
					<LoadingDefault color="#f3f6f9" size={50} />
				</div>
			{:else}
				{#each listImages as item}
					<div class="box br2 flex items-center justify-center mh3 relative">
						<img src={item.url} alt={item.id} class="image" />
						{#if listImages.length > 1}
							<div
								class="close bg-red flex items-center justify-center"
