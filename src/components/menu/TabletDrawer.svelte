...
<Portal target={document.getElementById('portal')}>
	...
	<div class="fixed z-max vh-100 bg-dark-blue top-0 w-40" transition:fade>
		<a
		...
		<a
			class="menu-link fw6 f4 h-60-px link white flex items-center spacing-ext"
		...
		{#if $isVendor && $auth}
			...
			<div
				class="menu-link fw6 h-50-px link white flex items-center spacing-ext"
				class:bg-light-yellow={$page.path.includes('/my-order')}
				class:dark-blue={$page.path.includes('/my-order')}
				on:click={() => goto('/my-order')}>
				{$_('order.myOrder')}
			</div>
		{:else}
			{#if isServerBrand}
			...
			{:else if brands.length > 0}
				{#each brands as brand}
					{#if $category}
						...
						{:else}
							<div
								class="menu-link fw6 f4 h-60-px link white flex items-center spacing-extr"
								on:click={() => handleCategory(brand)}
								title="Store">
								{brand.name}
							</div>
