...
{#if values}
	<div class="w-100">
		<h4 class="tc">{$_('checkout.billingAdd')}</h4>
		<div class="mt4 flex">
		...
		{#if !isSameShipping}
			<div class="mt4" id="firstNameBill">
				<label
					class="fw6 db"
			...
			<div class="mt4" id="addressBill">
				<label class="fw6 db" for="addressBill" class:red={errors.addressBill}
			...
			<div class="mt4" id="phoneBill">
				<label class="fw6 db" for="phoneBill" class:b--red={errors.phoneBill}
					>{$_('checkout.phoneNo')}</label
				>
				<PcMaskInput
					onInput={(value) => validate('phoneBill', value)}
					className={errors.phoneBill
						? 'ba b--red br2 fw6 f6 light-silver h-40-px mt3 ph1 w-small pl3'
