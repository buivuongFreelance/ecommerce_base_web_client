	...
	const onChange = (event) => {
		setTimeout(() => {
			let tt =
				...
				Number(
					convertCurrency(
						oldItem.selectedRate.amount,
						oldItem.selectedRate.currency,
						$settings
					)
				);
			...
			tt += Number(
				convertCurrency(
					item.selectedRate.amount,
					item.selectedRate.currency,
					$settings
				)
			);
			totalShip = tt;
							...
							<div class="">
								<label class="light-silver radio pointer">
									<input
										...
										type="radio"
										value={rate}
										bind:group={item.selectedRate} />
									<span class="box" />
									<span class="label fw6 ml3">{rate.provider}
