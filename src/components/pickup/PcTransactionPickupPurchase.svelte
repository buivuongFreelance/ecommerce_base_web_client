<script>
	...
	import {
		transactionPickupPurchase as apiTransactionPickupPurchase,
		detailOrder as apiDetailOrder,
		getShippingAndBilling as apiGetShippingAndBilling,
		createPickupPurchase as apiCreatePickupPurchase,
	...
	onMount(async () => {
		await transactionPickupPurchase();
		if (detail) {
			if (detail.length > 0) {
				await detailOrder();
			}
	...
	const transactionPickupPurchase = () => {
		isLoading = true;
		const token = $auth.accessToken;

		return new Promise((resolve, reject) => {
			apiTransactionPickupPurchase(domainTransaction, {
				token,
				id: $page.params.id,
			})
				.then((rs) => {
	...
	const createPickupPurchase = () => {
		isLoadingSubmit = true;
		const token = $auth.accessToken;

		return new Promise((resolve, reject) => {
			apiCreatePickupPurchase(domainTransaction, {
				token,
				id: $page.params.id,
				objPickup: values,
			})
	...
	const handleSubmit = async () => {
		...
		try {
			...
			await createPickupPurchase();
			await goto('my-purchases/' + $page.params.id);
			isLoadingSubmit = false;
		} catch (error) {
			isLoadingSubmit = false;
			if (error) {
