<script>
	...
	import { listModel as apiListModel } from '../../services/model';
	import config from '../../config';
	import LazyLoad from 'vanilla-lazyload';
	import { onDestroy } from 'svelte';
	import PcPagination from '../form/PcPagination.svelte';
	import { selectedModel } from '../../stores';
	import TabletModalSelectedModel from './TabletModalSelectedModel.svelte';
	...
	const handleSelectItem = (item) => {
		selectedModel.set(item);
	};
	const listModel = () => {
		isLoading = true;

		return new Promise((resolve, reject) => {
			apiListModel(domainModel, {
				limit: config.limit,
				offset,
				filter,
	...
	{:else if isLoading}
		<div class="pb5">
			<ContentLoader
				...
				width={800}
				height="200">
				<rect x="0" y="0" rx="3" ry="3" width="800" height="40" />
				<rect x="0" y="60" rx="3" ry="3" width="800" height="20" />
				<rect x="0" y="100" rx="3" ry="3" width="800" height="20" />
				<rect x="0" y="140" rx="3" ry="3" width="800" height="20" />
				<rect x="0" y="180" rx="3" ry="3" width="800" height="20" />
			</ContentLoader>
		</div>
	{:else if $selectedModel}
		<TabletModalSelectedModel
			onBack={() => selectedModel.set(null)}
			{onClose} />
	{:else}
