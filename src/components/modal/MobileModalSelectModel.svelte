<script>
	...
	import MobileModalListModel from './MobileModalListModel.svelte';
...
<div class="fixed z-max w-90 wrapper" transition:fade>
	<div class="w-100 bg-white container">
		<div
			class="flex items-center justify-between spacing-ext h-60-px flex
				items-center bg-light-gray b--pale-grey bb near-black fw5 f4 panel"
		>
			<div>{$_('message.selectItem')}</div>
			<div class="flex items-center" on:click={onClose}>
				<svg
		...
		<div class="spacing-ext content">
			...
			{:else}
				...
				{/if}
				<div class="mv4">
					<MobileModalListModel {selectedBrand} {onClose} />
				</div>
			{/if}
		</div>
	</div>
