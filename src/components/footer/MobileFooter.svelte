...
<div bind:this={containerEle} class={visible ? 'o-100' : 'o-0'}>
	<div class="footer pv6 lazy-bg" data-bg="images/bg-footer.png">
		<div class="spacing-ext tc">
			<div>
				<a href="/"><img
			...
			<div class="flex flex-wrap justify-center ttu mt5">
				<a class="fw6 white link dim pr4 pb4" href="/">{$_('nav.home')}</a>
				{#if isServerBrand}
				...
				{:else if isLoadingBrand}
					<ContentLoader
						...
						width="270"
						height="60">
						<rect x="0" y="0" rx="3" ry="3" width="270" height="60" />
					</ContentLoader>
				{:else if brands.length > 0}
					{#each brands as brand}
						<span
							class="fw6 white link dim ml3 pointer pr4 pb4"
				...
				{/if}
				<div
					class="fw6 white link dim pointer pr4 pb4"
					on:click={() => handleCategoryEmpty()}>
					{$_('nav.otherSmartPhones')}
				</div>
				<a href="/privacy" class="fw6 white link dim ph4 pointer pb4">
					{$_('checkout.privacy')}
				</a>
			</div>
			...
			<div class="text-stroke">Dingtoi Corporation 2021.</div>
		</div>
	</div>
</div>
