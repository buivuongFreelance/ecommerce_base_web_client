<script>
	...
	import TabletModalListModelWishlist from './TabletModalListModelWishlist.svelte';
	import MobileModalListModelWishlist from './MobileModalListModelWishlist.svelte';
					...
					{#each brands as item, i}
						<button
							...
							class="link br2 bn fw6 flex items-center brand justify-center
								h-50-px btn-width tracked">{item.name}</button>
					{/each}
				</div>

				<div class="mv4">
					<MobileModalListModelWishlist
						{selectedBrand}
						{onClose}
						{listExclude} />
				</div>
