...
{/if}
{#if item}
	<div class="offer flex pa4 bg-light-gray mt4">
					...
					{#if item.proposal_status == config.proposal.sellerAccept}
						...
						{:else}
							<button
								disabled={true}
								type="button"
								class="fw6 ph3 h-40-px br2 ba b--light-gray bg-white near-black flex items-center grow pointer o-50 mt3"
								>{$_('proposal.cancelAccept')}</button
							>
						{/if}
					{/if}
