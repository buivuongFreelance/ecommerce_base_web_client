<script>
	...
	import TabletOfferSellDetail from './TabletOfferSellDetail.svelte';
	import TabletOfferExchangeDetail from './TabletOfferExchangeDetail.svelte';
					...
					{#if detail.status !== config.proposal.sellerReply && detail.status !== config.proposal.sellerAccept}
						{#if !isUpdatePrice && !isUpdateExchangeDevices}
							<button
								type="button"
								disabled
								class="link bg-light-gray br1 ba fw6 ttu near-black
										b--pale-grey flex items-center justify-center
										h-50-px btn-width tracked o-50">{$_('offer.reply')}</button>
						...
						{:else}
							<button
								...
								class="link bg-light-gray br1 ba fw6 ttu near-black
										b--pale-grey pointer flex items-center justify-center grow
										h-50-px btn-width tracked">{$_('offer.reply')}</button>
						{/if}
					{/if}
