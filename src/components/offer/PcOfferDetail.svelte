<script>
	...
	import PcOfferExchangeDetail from './PcOfferExchangeDetail.svelte';
	import PcOfferSellDetail from './PcOfferSellDetail.svelte';
...
{#if isServerError}
	<p class="mt4 red f4">{$_('error.serverErrorMessage')}</p>
{:else if isLoading}
	<div class="mt4">
...
{:else if detail}
	<div class="flex mt5">
		<div class="mr5">
			<img src={detail.url} alt={detail.device_name} class="device-model br1" />
		</div>
		<div>
			<h2 class="mb3">
				{detail.device_name}
					...
					<!-- <div slot="header" class="flex">
						{#if detail.status == config.proposal.buyerReply || detail.status == config.proposal.buyerCreated}
							{#if isLoadingReject}
								<button
									type="button"
									class="bg-light-gray br2 ba fw6 h-40-px near-black
						...
						{#if isLoadingDelete}
							<button
								type="button"
								class="ml3 bg-light-gray br2 ba fw6 h-40-px near-black
										b--pale-grey pointer flex items-center justify-center grow
										pa3 tracked">
								<LoadingDefault size="25" color="#cccccc" />
							</button>
					...
					<!-- <div slot="header" class="flex">
						{#if detail.status == config.proposal.buyerReply || detail.status == config.proposal.buyerCreated}
							{#if isLoadingReject}
								<button
									type="button"
									class="link bg-light-gray br1 ba fw6 h-30-px f7 ttu near-black
											b--pale-grey pointer flex items-center justify-center grow
											pa3 tracked">
									<LoadingDefault size="25" color="#cccccc" />
								</button>
						...
						{#if isLoadingDelete}
							<button
								...
								type="button"
								class="ml3 link bg-light-gray br1 ba fw6 h-30-px f7 ttu near-black
										b--pale-grey pointer flex items-center justify-center grow
										pa3 tracked">
								<LoadingDefault size="25" color="#cccccc" />
							</button>
	...
	<div class="mb5 mt5 flex justify-between">
		<div>
			<button
				...
				type="button"
				class="link bg-light-gray br1 ba fw6 ttu near-black b--pale-grey
						pointer flex items-center justify-center grow h-50-px btn-width
						tracked">{$_('device.back')}</button>
		</div>
		{#if detail}
					...
					<div class="flex">
						...
						{:else}
							<button
								...
								pointer flex items-center justify-center grow h-60-px
								btn-width-large tracked f4">{$_('proposal.cancelAccept')}
								({minutes}
								:
								{seconds})</button>
						{/if}
				...
				{:else}
					<div class="flex">
						<button
							...
							class="link bg-light-gray br2 ba fw6 ttu near-black b--pale-grey
									pointer flex items-center justify-center grow h-60-px
									btn-width-large tracked f4 o-50">{$_('proposal.cancelAccept')}</button>
					</div>
				{/if} -->
			<div class="flex">
				{#if detail.status !== config.proposal.sellerReply && detail.status !== config.proposal.sellerAccept}
					{#if !isUpdatePrice && !isUpdateExchangeDevices}
						<button
							type="button"
							disabled
							class="link bg-dark-blue br1 ba fw6 ttu white
										b--dark-blue flex items-center justify-center
										h-50-px btn-width tracked o-50">{$_('message.submit')}</button>
					{:else if isLoadingComplete}
						<button
							type="button"
							class="link bg-dark-blue b--dark-blue br1 ba fw6 ttu white
										flex items-center justify-center grow h-50-px btn-width
										tracked">
							<LoadingDefault />
						</button>
					{:else}
						<button
							...
							class="link bg-dark-blue br1 ba fw6 ttu white
										b--dark-blue pointer flex items-center justify-center grow
										h-50-px btn-width tracked">{$_('message.submit')}</button>
					{/if}
				{/if}
				<!-- {#if detail.status !== config.proposal.sellerReply}
						{#if !isUpdatePrice && !isUpdateExchangeDevices}
							...
							{:else}
								<button
									...
									class="link bg-dark-blue b--dark-blue br1 ba fw6 ttu white
										flex pointer items-center justify-center grow h-50-px
						...
						{:else}
							<button
								...
								disabled
								class="link bg-dark-blue b--dark-blue br1 ba fw6 ttu white
								flex items-center justify-center h-50-px btn-width tracked ml3 o-50">{$_('message.accept')}</button>
						{/if}
					{/if} -->
				{#if detail.status === config.proposal.sellerReply}
					{#if isLoadingCancelReply}
						<button
							type="button"
							class="link bg-dark-blue b--dark-blue br1 ba fw6 ttu white
										flex items-center justify-center grow h-50-px btn-width tracked ml3">
							<LoadingDefault />
						</button>
					{:else}
						<button
