	...
	<div
		...
		<div class="content">
			<div class="flex justify-between">
				...
				{#if detail.status !== config.proposal.sellerAccept && detail.status !== config.proposal.sellerReply}
					<button
						...
						class="link bg-light-gray br1 ba fw6 h-50-px near-black
			b--pale-grey pointer flex items-center justify-center dim ph6 ttu">{$_('message.reset')}</button>
				{/if}
			</div>
			<div class="mv4">
			...
			{:else}
				<div class="flex flex-column items-center justify-center">
					{#if detail.status !== config.proposal.sellerAccept && detail.status !== config.proposal.sellerReply}
						<div class="mb4 mv6 money-exchange money-slider-sell-pc">
							<PcMoneySliderTwo
								bind:bindValue
								onUpdate={handleUpdate}
