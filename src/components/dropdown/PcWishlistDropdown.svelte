<script>
	...
	import { auth, anonymous, favoriteChanged, cartChanged } from '../../stores';
	import LoadingDefault from '../loading/LoadingDefault.svelte';
	import { onDestroy, onMount } from 'svelte';
	import PcWishlistItemDropdown from '../wishlist/PcWishlistItemDropdown.svelte';
	...
	const handleMenuOpen = () => {
		if (list.length > 0) {
			...
			const boundingClientRect = wishEle.getBoundingClientRect();
			top =
...
<div
	...
	{:else}
		<div
			...
			{#if isOpenMenu}
				<PcWishlistItemDropdown {top} {left} {list} />
			{/if}
		</div>
	{/if}
</div>
