...
{#if isOpen}
	<Portal target={document.getElementById('overlay')}>
		...
		<ul
			...
			style="--top: {top - 12}px; --left: {left}px">
			{#if $auth}
				...
				<li
					class="lh-copy"
					on:click={async () => await goto('my-notification')}>
					<div class="pv3 spacing-ext flex items-center self-center">
						<svg
							fill="#3F4254"
							xmlns="http://www.w3.org/2000/svg"
						...
						<span class="ml3 near-black fw6">{$_('nav.notification')}</span>
					</div>
				</li>
...
{/if}
<div
	class="pl3 h-60-px flex items-center relative"
	on:click={handleClick}
	bind:this={rootEle}>
	<span class="fw5 br2 flex items-end">
		<svg
			xmlns="http://www.w3.org/2000/svg"
			viewBox="0 0 24 24"
			fill="white"
