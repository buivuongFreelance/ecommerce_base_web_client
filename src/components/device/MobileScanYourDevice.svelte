...
{:else if isLoading}
	<div>
		<ContentLoader width="345" uniqueKey="scanYourDevice">
...
{:else if detail && downloadQrCodeLink && scanDeviceQrCodeLink}
	...
	{#if listScan.length === 0 || isChecked === config.no}
		{#if step >= 1}
			...
			<div class="">
				<div class="fw6 f4 light-silver">{$_('message.qrStep1')}</div>
				<div class="flex flex-column mv5 items-center">
					{#if detail.brand_name === 'Apple'}
					...
					<div class="flex items-center">
						<div class="light-gray f4 mv4 fw6 mh6">{$_('message.or')}</div>
					</div>
		...
		{#if step === 3}
			...
			<div class="flex items-center flex-column">
				...
				<div class="mt3 tc light-silver">{$_('image.qrDeviceDesc')}</div>
				<div class="mv4">
					<img src={scanDeviceQrCodeLink} alt="" width="140px" />
				</div>
				<div class="mt3 tc light-silver">{$_('device.qrCodeDesc')}</div>
			</div>
		{/if}
		{#if step !== 2}
			<div
