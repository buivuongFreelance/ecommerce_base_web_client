<script>
	...
	import TabletPostedDeviceStep from './TabletPostedDeviceStep.svelte';
	import TabletDeviceShippingBilling from './TabletDeviceShippingBilling.svelte';
...
{:else if detail}
	<h2 class="ttc tc">{$_('device.editPostDevice')}</h2>
	<p class="tc f3 fw5 light-silver">{detail.model}</p>

	<div class="mt6">
		<TabletPostedDeviceStep {step} onStep={(st) => (step = st)} />
	</div>
	<div class="w-100">
		{#if step === 1}
		...
		{:else if step === 4}
			<div class="flex justify-center">
				<div class="mt6 w-738px">
					<TabletDeviceShippingBilling
						bind:step
