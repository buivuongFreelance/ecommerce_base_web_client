<script>
	...
	import TabletDeviceAccountItem from './TabletDeviceAccountItem.svelte';
...
{:else}
		...
		<div class="justify-between flex flex-wrap">
			...
			{:else}
				<form
					...
					<PcButtonSearch bg="bg-blue" className="mh3" />
	...
	<div class="pb5 mt4 w-100 spacing-ext">
		{#if isLoading}
			<ContentLoader
		...
		{:else}
			<div class="pb6 mt4 w-100 spacing-ext f4">{$_('message.noItems')}</div>
		{/if}
	</div>
{/if}
