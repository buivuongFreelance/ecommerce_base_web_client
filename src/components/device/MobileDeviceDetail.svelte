<script>
	...
	import MobileDeviceImages from './MobileDeviceImages.svelte';
	import * as animateScroll from 'svelte-scrollto';
	import IconSortDown from '../icon/IconSortDown.svelte';
	...
	const handleGoToTab = (tab, accordion) => {
		accordion1 = accordion === 'tab__report' ? true : false;
		accordion2 = accordion === 'tab__info' ? true : false;
		accordion3 = accordion === 'tab__accessories' ? true : false;
		accordion4 = accordion === 'tab__grading' ? true : false;

		setTimeout(() => {
			animateScroll.scrollTo({ element: tab, offset: -100 });
		}, 0);
	};
	...
	{#if item}
		<div class="custom">
			<div class="header mb4">
				...
				<div class="subtitle fw6">
					<div class="mt1 flex flex-wrap items-center">
						<span
							class="grade mr3 dark-blue"
							on:click={() => handleGoToTab(accessoriesEle, 'tab__grading')}
						>
							{$_('device.grade')}
							{item.physical_grading}
							<IconHelp size="12" fill="#2f3a97" />
						</span>
						<PcRating rating={item.dingtoi_rating} />
						<div
							class="light-blue u flex items-center ml3"
							on:click={() => handleGoToTab(reportEle, 'tab__report')}
						>
							<span>{$_('device.viewScanReport')}</span>
							<IconSortDown fill="#3699ff" />
						</div>
					</div>
				</div>
			</div>
			<div class="body">
				<div class="device-detail-container mb6">
					<div class="device-image mb4">
						{#if item.images}
							{#if item.images.length > 0}
								<div class="image-container">
									<div class="image_main">
										<MobileDeviceImages images={item.images} />
									</div>
								</div>
							{/if}
						{/if}
					...
					<div class="device-content">
								...
								<div class="ml-40px">
									...
									<div class="flex mt3">
										...
										<div class="ml4">
											...
											>
												<div class="i">{$_('message.exchangeWith')}</div>
												<b>
						...
						<div class="specifications mb4">
							<div class="tc">
								<div
									...
									on:click={() => handleGoToTab(seeDetailEle, 'tab__info')}
								>
						...
						<div class="mt4 mb3 flex items-center">
							...
							<div class="blue fw6 f4 ml3 dim ttu" on:click={handleGoToCompare}>
								{$_('label.compare')}
							</div>
						</div>
			...
			<div class="footer">
				<div class="accordions">
					<div class="accordion mb3">
						<div
							class="accordion-header flex items-center h-50-px ttu bb b--light-gray dark-blue"
							bind:this={reportEle}
							on:click={() => (accordion1 = !accordion1)}
						>
							<h5 class="accordion-title flex-auto b">
								{$_('device.dingtoiScanReport')}
							</h5>
							<span
								class="accordion-collapse"
								class:opened={accordion1 ? true : false}
							/>
						</div>
						{#if accordion1}
							<div class="accordion-body pv4">
								{#if item.main_info}
									<h5 class="">
					...
					<div class="accordion mb3">
						<div
							class="accordion-header flex items-center h-50-px ttu bb b--light-gray dark-blue"
							bind:this={warrantyEle}
							on:click={() => (accordion3 = !accordion3)}
						>
							<h5 class="accordion-title flex-auto b">
								{$_('device.warranty')}
								&
							...
							<span
								class="accordion-collapse"
								class:opened={accordion3 ? true : false}
							/>
						</div>
						{#if accordion3}
							<div class="accordion-body pv4">
								{#if item.is_warranty}
									<h5 class="fw6 mb3">{$_('device.timeRemaining')}</h5>
									<div class="mb4">
					...
					<div class="accordion mb3">
						<div
							class="accordion-header flex items-center h-50-px ttu bb b--light-gray dark-blue"
							bind:this={seeDetailEle}
							on:click={() => (accordion2 = !accordion2)}
						>
							<h5 class="accordion-title flex-auto b">
								{$_('device.moreInfo')}
							</h5>
							<span
								class="accordion-collapse"
								class:opened={accordion2 ? true : false}
							/>
						</div>
						{#if accordion2}
							<div class="accordion-body pv4">
								<div class="mb3 f5 b">{$_('deviceDetail.mainInfo')}</div>
					...
					<div class="accordion">
						<div
							class="accordion-header flex items-center h-50-px ttu bb b--light-gray dark-blue"
							bind:this={accessoriesEle}
							on:click={() => (accordion4 = !accordion4)}
						>
							<h5 class="accordion-title flex-auto b">
								{$_('deviceDetail.grading')}
							</h5>
							<span
								class="accordion-collapse"
								class:opened={accordion4 ? true : false}
							/>
						</div>
						{#if accordion4}
							<div class="accordion-body pv4">
...
{:else}
	<div class="flex justify-center mv4">
		<ContentLoader width="375" uniqueKey="device-detail">
...
<style>
	...
	.accordion-collapse.opened:after {
		transform: rotate(45deg);
	}
