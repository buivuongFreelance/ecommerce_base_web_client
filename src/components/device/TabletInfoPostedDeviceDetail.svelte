...
{#if visible}
	<div
		class="h-60-px white bg-dark-blue b--light-gray br--top br2 flex
		...
		<div class="w-70">
				...
				>
					<div class="flex">
						<div>
							...
							<div class="steel-two tr">{$_('device.youAskingPrice')}</div>
						</div>
						...
						<ul class="near-black d-list-item">
							<li class="">
								...
								{#if !isPastDate($infoImei.warranty_expire_date)}
									<span
										>{displayDateDefault($infoImei.warranty_expire_date)}</span
									>
			...
			{#if !is.empty($infoImei.accessories) && $infoImei.accessories}
				{#if !isObjectEmpty($infoImei.accessories)}
					<div class="mt4 mb3 b f4 near-black">{$_('device.accessories')}</div>
			...
			{#if $infoImei.status !== config.device.inTransaction}
				<div class="flex justify-end mb5 mt4">
					<button
						type="button"
						on:click={handleGotoEdit}
						class="br2 bg-yellow b--yellow ba grow ttu h-50-px fw6 w-50 tracked
						near-black pointer"
					>
					...
					{#if isLoadingDelete}
						<button
							type="button"
							class="br2 bg-red b--red ba grow ttu h-50-px fw6 w-50 tracked ml3
							white pointer"
						>
