<script>
	...
	import PcDropdownMoreMenuAccoutItemDevice from './PcDropdownMoreMenuAccoutItemDevice.svelte';
...
{#if item}
	<tr class="ba b--black-10 bt-0 bl-0 br-0 relative">
		<td class="">
			...
			<a href={'my-devices/' + item.id} class="db link light-blue fw6"
				>{item.model}
				{item.color}
				{item.capacity}
				{$_('label.GB')}</a
			...
			<div class="flex justify-end">
				...
				{:else if item.transaction_type == config.buyer}
					<PcButtonTransactionProcessing
						onClickButton={() =>
							goto(`/my-transactions/${item.transaction_id}`)}
					/>
					<div class="w40px ml3" />
				...
				{:else}
					<PcButtonTransactionProcessing
						onClickButton={() =>
							goto(
								`/my-transactions/${
									item.transaction_email_buyer == $auth.email
										? item.transaction_exchange_id
										: item.transaction_id
								}`
							)}
					/>
