<script>
	...
	import MobilePostDeviceImages from './MobilePostDeviceImages.svelte';
...
{#if visible}
	<h4 class="tc ttc mt2rem">
		{$_('device.postDeviceInformation')}
	</h4>
	<div class="mt1rem">
		<div class="w-100">
			<div class="mt4">
				<MobilePostDeviceImages images={$infoImei.images} />
			</div>
		</div>
		{#if $infoImei}
		...
		{#if $infoImei.real_sale_price > 0}
			<div
				class="flex items-center justify-around b--pale-grey ba br2 pa1rem mv1rem"
			>
				<div class="tr light-silver">
					<div class="f4">{$_('device.sellingPrice')}</div>
					<u class="db">{$currency}</u>
				</div>
				<span class="b f2 near-black">
					{displayCurrency($infoImei.real_sale_price, $currency)}
				</span>
			</div>
			<div class="flex justify-end">
				<button
					class="dib bn h-40-px ph1rem bg-dark-blue white br2 grow pointer fw6"
		...
		{#if $infoImei.exchange_model}
			<div
				class="w-100 mt4 b--pale-grey ba br10 pv4 flex items-center
					justify-center"
			>
				<div class="flex items-center">
					<div>
			...
			<div class="mt4 near-black b f4">{$_('device.deviceExchangeInfo')}</div>
			<div class="mt3">
				<div class="flex items-center">
					...
					<div>
						...
						<div class="mt3">
							{$_('device.strCa')}:
							<span class="fw6">{$infoImei.exchange_model.capacity_name}</span>
						</div>
					</div>
				</div>
			</div>
		...
		{#if $infoImei.is_warranty === config.yes}
			{#if $infoImei.warranty_expire_date}
				<div class="mt4 f5 b near-black">{$_('device.warranty')}</div>
				<div class="mt3">
					<ul class="near-black d-list-item">
						<li class="">
							...
							{#if !isPastDate($infoImei.warranty_expire_date)}
								<span>{displayDateDefault($infoImei.warranty_expire_date)}</span
								>
		...
		{:else}
			<div class="mt2rem f5 b near-black">{$_('device.warranty')}</div>
			<div class="mt3">
				<ul class="near-black d-list-item">
					<li class="">
						<b class="red i">{$_('device.expired')}</b>
						<span class="ph1 gray">:</span>
		...
		{#if !is.empty($infoImei.accessories) && $infoImei.accessories}
			{#if !isObjectEmpty($infoImei.accessories)}
				<div class="mt2rem mb3 b f5 near-black">{$_('device.accessories')}</div>
				...
				{#if accessories.length > 0}
					<div class="flex flex-wrap">
						{#each accessories as accessory}
							{#if accessory.value}
								<div
									class="accessory br1 bg-white ba b--dark-blue dark-blue ph4
										pv3 mr3 mb3"
								>
		...
		{#if !is.empty($infoImei.additional_accessories) && $infoImei.additional_accessories}
			<div class="mt4 f4 b w-100">{$_('device.additionalAccessories')}</div>
			<div class="mt3 w-100">{$infoImei.additional_accessories}</div>
		{/if}
		{#if $infoImei.status !== config.device.inTransaction}
			<div class="mb5 mt4">
				<button
					...
					class="br2 bg-yellow b--yellow ba grow ttu h-40-px fw6 w-100 tracked
					near-black pointer mb3"
				>
