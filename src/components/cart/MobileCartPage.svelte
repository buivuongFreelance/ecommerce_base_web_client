...
{:else}
	...
	<div class="sticky-bottom mt4 bg-white flex items-center justify-between">
		<div class="pl1rem">
			{#if flagSelectAll === true}
				<label class="checkbox flex items-center pointer ">
					...
					/>
					<span class="box flex items-center justify-center" />
					<span class="label f7 ml3">{$_('message.selectAll')}</span>
				</label>
			{/if}
		</div>
		<div class="flex items-center">
			{#if selectItemCarts.length > 0 && $auth}
				<div class="f6 tr pr3">
					...
					{:else}
						<span class="fw6">
							{$_(displayProposalBuyerExchangeStr(total))}
						</span>
						<span class="b price">
							{displayCurrency(total, $currency)}
							<u>{$currency}</u>
			...
			{#if listConfirm.length > 0}
				{#if isLoadingConfirm}
					<button
						type="button"
						class="bg-light-yellow b--light-yellow ba fw6 ttu dark-blue flex
							items-center justify-center h-60-px tracked f4"
					>
						<LoadingDefault />
					</button>
				{:else if selectItemCarts.length <= 0}
					<button
						type="button"
						disabled
						class="bg-light-yellow bn fw6 ttc dark-blue flex
							items-center justify-center h-60-px ph1rem o-50"
					>
						{$_('cart.checkOut')}
				...
				{:else}
					<button
						...
						class="bg-light-yellow bn fw6 ttc dark-blue flex
							items-center justify-center h-60-px ph1rem"
					>
						{$_('cart.checkOut')}
						({selectItemCarts.length})
					</button>
	...
	<!-- <button
		...
		class="bg-light-gray b--light-gray br1 ba fw6 ttu flex items-center
				justify-center h-60-px btn-block tracked near-black mt3"
