...
import MobileOfferExchange from './MobileOfferExchange.svelte';
	...
	const init = () => {
		if (detail.status.includes(config.buyer)) {
			exchangeDevices = detail.exchange_devices;
			exchangePrice = displayCurrencyDb(detail.buyer_exchange_price, $currency);
		} else {
			exchangeDevices = detail.exchange_devices;
			...
			if (detail.type == config.exchange) {
				...
				} else typeExchange = config.exchangeType.offer;
			}
		}
	};
				...
				textDesc = getContentSellAcceptCartSellToExchange({
					action:
						typeExchange == config.exchangeType.request
							...
							: $_('cart.exchangePay'),
					devices: detail.exchange_devices,
					money_exchange: displayCurrency(exchangePrice, $currency),
					price: displayCurrency(detail.device_sale_price, $currency),
				});
			}
			try {
				if (cart.cart_type == config.sell) {
	...
	<div class="w-100 bg-white container">
		...
		<div class="ph4 content">
			...
			{:else}
				...
				{/if}
				<div class="mv4 flex justify-between items-center">
					{#if detail}
						{#if detail.type == config.exchange && detail.status !== config.proposal.systemCancelAccept}
							<span>
								{$_('proposal.lastOffer')}:
								{#if detail.buyer_real_exchange_price < 0}
									{$_('proposal.yourLastOffer')}
									{displayCurrency(detail.buyer_real_exchange_price, $currency)}
									{$currency}
								{:else}
									{$_('proposal.yourLastRequest')}
									{displayCurrency(detail.buyer_real_exchange_price, $currency)}
									{$currency}
								{/if}
							</span>
