...
{:else}
	<div class="box-shadow flex pb4 bg-light-gray pa4 br2 mt3 justify-end">
		{#if item.validate}
			<slot />
		{/if}
		<div class="ba bw2 b--white flex items-center justify-center w-image ml4">
		...
		<div class="device-info ml4 flex justify-between">
			<div>
				...
				{#if item.is_proposal_accepted && item.proposal_status !== config.proposal.sellerAccept}
					<div class="bg-blue-20 dark-blue ph4 pv3 mt3 br1">
							...
							{#if item.proposal_id}
								{#if item.proposal_status == config.proposal.sellerReject || item.proposal_status == config.proposal.systemCancelAccept}
									<div
										...
										class="dim pointer flex items-center"
									>
										<span
							...
							{#if item.proposal_id}
								{#if item.proposal_status == config.proposal.sellerReject || item.proposal_status == config.proposal.systemCancelAccept}
									<div
										...
										class="dim pointer flex items-center ml-40px"
									>
										<span
											class="h-40-px w2 bg-dark-blue br2 flex items-center justify-center"
										>
