<script>
	...
	import MobileModalSelectDeviceOffer from './MobileModalSelectDeviceOffer.svelte';
	...
	const initExchange = () => {
		if (listDeviceMatch.length > 0) {
			...
			// let tempCapacity = listDeviceMatch[0].capacity;
			// const filterListDeviceMatch = [];
			// const selectedDeviceMatch = null;
			// listDeviceMatch.map((ldm) => {
			// 	const capacityCart = Number(
			// 		cart.exchange_model.capacity_name.split(' ')[0]
			// 	);
			// 	const colorCart = cart.exchange_model.color_name;
			// 	const capacityLdm = Number(ldm.capacity);
			// 	const colorLdm = ldm.color;
			// 	if(capacityCart === capacityLdm){
			// 		selectedDeviceMatch = ldm;
			// 	}else{
			// 	}
			// 	console.log(capacityCart, colorCart, capacityLdm, colorLdm);
			// });
		}
...
{#if isOpenModal}
	<Portal target={document.getElementById('portal')}>
		...
		<MobileModalSelectDeviceOffer
			listAdded={listServerExchangeDeviceRemoved}
			selectedExchangedDevices={selectedExchangeDevices}
			onClose={handleCloseModal}
			filterModel={cart.cart_type === config.exchange &&
			listDeviceMatch.length > 1
				? cart.exchange_model.model_name
				: ''}
		...
		<div class="mt4">
			...
			{:else if listDeviceMatch.length > 1}
				<div class="i mb4">
					...
					<a href="javascript:void(0);" on:click={onSelectDeviceOffer}
						>clicking here</a
					>
				</div>
			{:else}
			...
			{/if}
			<div class="w-100 flex justify-center flex-wrap flex-gap10px mt3">
				{#if isLoading}
				...
				{:else}
					{#each selectedExchangeDevices as exchangeDevice}
						<div
							on:click={() =>
								listDeviceMatch.length > 1 ? onSelectDeviceOffer() : null}
						...
						>
							...
							{/if}
							<img
								src={exchangeDevice.url
									? exchangeDevice.url
									: exchangeDevice.model_url}
								alt=""
								class="image-device ba b--light-gray"
							/>
							<div class="f7 mt1 name">
								{exchangeDevice.model}
								{exchangeDevice.capacity}{$_('label.GB')}
			...
			<div class="mt4">
				...
				<div class="mt3">
					...
					<div class="flex mt4">
						...
						<label
							for="inputNumber"
							class="flex items-center justify-center f3 ml3 b"
							>{$currency}</label
						>
					</div>
				</div>
