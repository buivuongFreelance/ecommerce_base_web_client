<script>
	...
	import PcMyAccountCreditCard from '../../components/myAccountCreditCard/PcMyAccountCreditCard.svelte';
...
{#if visible}
	<TransactionWrapper id="my-account-screen">
		{#if deviceType === config.isMobile}
			<MobileNav />
			<div class="content-mobile">mobile my account</div>
		{:else if deviceType == config.isTablet}
			<TabletNav />
			<div class="content-tablet">tablet my account</div>
		{:else}
			<PcNav />
			<div class="content-desktop">
				<Breadcrumb>
					<BreadcrumbItem active={true} message={$_('account.myAcc')} />
				</Breadcrumb>
				<section class="spacing-ext mw10 center pv5">
					<PcMyAccountCreditCard />
				</section>
			</div>
		{/if}
